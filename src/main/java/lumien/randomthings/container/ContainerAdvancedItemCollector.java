...
public class ContainerAdvancedItemCollector extends Container
{
	World worldObj;
	BlockPos pos;
	TileEntityAdvancedItemCollector advancedItemCollector;

	public ContainerAdvancedItemCollector(EntityPlayer player, World world, int x, int y, int z)
	{
		this.worldObj = world;
		this.pos = new BlockPos(x, y, z);
		this.advancedItemCollector = (TileEntityAdvancedItemCollector) world.getTileEntity(pos);

		this.addSlotToContainer(new SlotFiltered(advancedItemCollector.getInventory(), 0, 77, 122, new Predicate<ItemStack>()
		{
			@Override
			public boolean apply(ItemStack input)
			{
				return input.getItem() == ModItems.itemFilter;
			}
		}));
		...
		if (par1ItemStack.isStackable())
		{
			while (par1ItemStack.getCount() > 0 && (!par4 && k < par3 || par4 && k >= par2))
			{
				...
				if (!itemstack1.isEmpty() && itemstack1.getItem() == par1ItemStack.getItem() && (!par1ItemStack.getHasSubtypes() || par1ItemStack.getItemDamage() == itemstack1.getItemDamage()) && ItemStack.areItemStackTagsEqual(par1ItemStack, itemstack1) && slot.isItemValid(par1ItemStack))
				{
					...
					else if (itemstack1.getCount() < par1ItemStack.getMaxStackSize() && itemstack1.getCount() < slot.getSlotStackLimit())
					{
